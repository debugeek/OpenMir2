...
func TestTeleportToWalkableTile(t *testing.T) {
	...
	mp := data.StdMap{ID: "map-1", Width: 4, Height: 4, SpawnX: 1, SpawnY: 1}
...
func TestTeleportToBlockedTileFallsBackToSpawnArea(t *testing.T) {
	ch := storage.Character{ID: "char-1", MapID: "old-map", X: 1, Y: 1}
	mp := data.StdMap{ID: "map-1", Width: 4, Height: 4, SpawnX: 1, SpawnY: 1, Blocked: []data.StdPoint{{X: 3, Y: 3}}}
...
func TestTeleportRandomInMapMovesCharacter(t *testing.T) {
	...
	mp := data.StdMap{ID: "map-1", Width: 2, Height: 2, SpawnX: 0, SpawnY: 0}
